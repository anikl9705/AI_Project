...
class Maze:
	...
	#Method to add description of house (delivery location)
	def add_house_description(self, f_out, coords):
			for i in coords:
				x, y = i
				f_out.write('<model name=\'House{}{}\'>\n'.format(x, y))
	...
	def generate_maze(self, grid_dimension, n_obstacles, seed, scale=0.5):
		...
		while(count <= n_obstacles):
			x = scale*np.random.randint(0, grid_dimension+5)
			y = scale*np.random.randint(0, grid_dimension+5)
			...
			if(flag == 0 and ((x+scale) <= grid_dimension*scale) and ((x, y, x+scale, y) not in blocked_edges)):
				...
				self.add_house(f_out, x+scale/2+offset, y)
				count += 1
				...
				self.add_house(f_out, x, y+scale/2-offset)
				count += 1
		...
		self.add_house_description(f_out, coords)
		f_out.write('</world>\n</sdf>')
		f_out.close()
